		...
		wrappers.DataUsageSync(usage.NewDataUsageSyncer()), &info.InfoImpl{
			Info: &plugin.PluginInfo{
				...
				Parameters: []*plugin.ParameterInfo{
					...
					{Name: constants.AwsS3IncludeBuckets, Description: "Optional comma-separated list of buckets to include. If specified, only these buckets will be handled. Regular expressions can be used (e.g. 'dev-.*,test-bucket').", Mandatory: false},
					{Name: constants.AwsS3ExcludeBuckets, Description: "Optional comma-separated list of buckets to exclude. If specified, these buckets will not be handled. Regular expressions can be used (e.g. 'prod-.*,legacy-bucket'). Excludes have preference over includes.", Mandatory: false},
					...
					{Name: constants.AwsGlueExcludeDatabases, Description: "Optional comma-separated list of Glue databases to exclude. If specified, these databases (and all their tables) will not be handled. Regular expressions can be used (e.g. 'test-db-.*,prod-db').", Mandatory: false},
					{Name: constants.AwsGlueExcludeTables, Description: "Optional comma-separated list of Glue tables to exclude. If specified, these tables will not be handled. Regular expressions can be used (e.g. 'temp-.*,sensitive-table'). This applies to all databases that are not excluded.", Mandatory: false},
					// Access parameters
					...
					{Name: constants.AwsPermissionSetDummyUser, Description: "Optional dummy user to use for permission sets that are not associated with any user or group. A permission set in AWS cannot be provisioned to an AWS account when no users are assigned to it, so this user will be assigned to avoid this situation to make sure that the provisioned permission set can be referenced by other Access Controls.", Mandatory: false},
				},
			},
		})
